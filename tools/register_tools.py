...
def _get_category(tool_name: str) -> str:
    code_engine_names = {
        "list_code_engine_projects", "list_code_engine_apps", "get_app_details",
        "get_app_revisions", "get_app_instances", "get_app_logs", "check_app_health",
        "update_app_env_vars", "update_app", "restart_app", "create_app",
        "delete_app", "list_jobs", "create_job_run", "get_job_run_status",
    }
    if tool_name in code_engine_names:
        return "Code Engine"
    if "log" in tool_name or "instance" in tool_name:
        return "Cloud Logs"
    if "monitor" in tool_name or "metric" in tool_name or "alert" in tool_name or "dashboard" in tool_name:
        return "Cloud Monitoring"
    if "database" in tool_name or "backup" in tool_name or "connection" in tool_name or "whitelist" in tool_name:
